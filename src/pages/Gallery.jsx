...
const Gallery = () => {
  ...
  const handleImageError = useCallback((index, e) => {
    ...
    if (container) {
      ...
      container.innerHTML = '<div class="flex items-center justify-center h-full text-gray-400">Image not available</div>';
    }
  }, []);
                ...
                {images.slice(0, 10).map((image, index) => {
                  ...
                  return (
                    <div
                      key={index}
                      ref={(el) => {
                        if (el && observerRef.current && !loadedImages.has(imageIndex)) {
